...
export const getAllTodos = async (dispatch) => {
  ...
  try {
    const res = await axios.get("http://localhost:9090" + "/todos/todos-all");
...
export const createTodo = (payload) => async (dispatch) => {
  ...
  try {
    const res = await axios.post("http://localhost:9090" + "/todos/todo/new", {
...
export const deleteTodo = (payload) => async (dispatch) => {
  ...
  try {
    const res = await axios.delete(
      "http://localhost:9090" + "/todos/todo/" + payload.todoId
    );
